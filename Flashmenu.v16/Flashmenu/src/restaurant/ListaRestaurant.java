...
public class ListaRestaurant extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			...
			//	String id_resturant = TAG_ID;
				// Starting new intent
				Intent in = new Intent(getApplicationContext(),	perfilRestaurant.class);
				in.putExtra("idADM",idADM);
				in.putExtra("nomRest",cedula);
				in.putExtra("idREST",TAG_ID);
			//	in.putExtra("nom",nombreR);
				
				
				// sending pid to next activity		
				in.putExtra(TAG_NOMBRE, cedula);
			//	in.putExtra(TAG_ID, id_resturant);
	...
	class LoadAllempleados extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			...
			pDialog.setMessage("Cargando restaurantes. Please wait...");
			...
			try {
				...
				if (success == 1) {
					...
					//	String cedula = c.getString(TAG_CEDULA);
						nombreR = c.getString(TAG_NOMBRE);
